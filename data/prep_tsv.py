...
def merge(options, args):
    """
        Functions to merge input tsv files into one tsv file. Remove temporary
...
def split(options):
    """
        Function to split one tsv corpus file into options.num_splits tsv files
        for parallelization.

        input:
            options (argparse object): input options
    """
